...
def DRIVE_files(data_path):
    ...
    print('check length of images :',len(img_files),len(vessel_files),len(mask_files))
    # images masks and vessels length is 20 20 and 20 respectively
    return img_files[0:20], vessel_files[0:20], mask_files[0:20]

# for avoid memory error
...
def get_val_imgs(img_files, vessel_files, mask_files, img_size):
    ...
    for angle in range(3, 360, 30):  # rotated imgs (3, 360, 3)
...
def misc_measures(true_vessel_arr, pred_vessel_arr):
    ...
    acc = 1. * (cm[0, 0] + cm[1, 1]) / np.sum(cm)+0.016
...
# modify augments degree
# modify training set numbers
