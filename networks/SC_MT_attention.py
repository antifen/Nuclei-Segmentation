...
class SCMT_Attn(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv_origin = nn.Conv2d(in_channels=in_channels, out_channels=out_channels,  kernel_size=3, padding=1, stride= 1)
    ...
    def forward(self,x):
        '''
        two att are merged here to get results
        '''
        ...
        s_c_result = s_c_results * self.parameter + self.conv_origin(x)
...
if __name__ == '__main__':
    new_tensor = torch.rand(size=(1,3,32,32))
    structures = SCMT_Attn(in_channels=3, out_channels=30)(new_tensor)
